...
const resolveFrom = (fromDirectory, moduleId, silent) => {
	...
	try {
		fromDirectory = __chunk_2.fs.realpathSync(fromDirectory);
	} catch (error) {
		if (error.code === 'ENOENT') {
			fromDirectory = __chunk_1.path.resolve(fromDirectory);
		} else if (silent) {
			return;
		} else {
			throw error;
	...
	const fromFile = __chunk_1.path.join(fromDirectory, 'noop.js');

	const resolveFileName = () => __chunk_1.require$$0._resolveFilename(moduleId, {
		id: fromFile,
		filename: fromFile,
		paths: __chunk_1.require$$0._nodeModulePaths(fromDirectory)
	});
...
// TODO: Remove this for the next major release
var default_1$1 = pWaterfall;
pWaterfall_1.default = default_1$1;
...
var nodeResolvePlugin = (options => {
  const plugin = require('rollup-plugin-node-resolve')({
    ...
    preferBuiltins: true,
    mainFields: [options.browser && 'browser', 'module', 'jsnext:main', 'main'].filter(Boolean)
  });
...
const printAssets = _async$1(function (assets, title) {
  return _await$1(Promise.resolve(require('./chunk-9b032883.js')).then(res => res.default), function (gzipSize) {
...
class Bundler {
  constructor(config, options = {}) {
    ...
    if (/\.mjs$/.test(this.pkg.data.module || this.pkg.data.main)) {
      logger.warn(`Bili no longer use .mjs extension for esm bundle, you should use .js instead!`);
    }
  ...
  createRollupConfig({
    ...
    return _call(function () {
      ...
      const bundleNodeModules = rollupFormat === 'umd' || rollupFormat === 'iife' || config.bundleNodeModules; // rollup-plugin-typescript2 < v0.26 needs the `objectHashIgnoreUnknownHack`
      ...
      const getObjectHashIgnoreUnknownHack = () => {
        try {
          const {
            version
          } = _this.localRequire('rollup-plugin-typescript2/package.json');
      ...
      const env = Object.assign({}, config.env); // drop precess.env.NODE_ENV from umd/iife

      if (['umd', 'umd-min', 'iife', 'iife-min'].includes(format) && env.NODE_ENV === undefined) {
        env.NODE_ENV = minify ? 'production' : 'development';
      }

      pluginsOptions.replace = Object.assign(Object.assign({}, Object.keys(env).reduce((res, name) => {
      ...
      if (minify) {
        ...
        pluginsOptions.terser = Object.assign(Object.assign({}, terserOptions), {
          output: Object.assign(Object.assign({}, terserOptions.output), {
      ...
      const getPlugin = name => {
        ...
        const isLegacyBuiltIn = require('../package').dependencies[`rollup-plugin-${name}`];

        const isBuiltIn = require('../package').dependencies[`@rollup/plugin-${name}`];

        const plugin = name === 'babel' ? Promise.resolve(require('./chunk-e56d592a.js')).then(res => res.default) : name === 'node-resolve' ? nodeResolvePlugin : name === 'progress' ? progressPlugin : name.startsWith('@rollup/') ? _this.localRequire(name) : isLegacyBuiltIn ? require(`rollup-plugin-${name}`) : isBuiltIn ? require(`@rollup/plugin-${name}`) : _this.localRequire(`rollup-plugin-${name}`);
      ...
      return _await$1(Promise.all(Object.keys(pluginsOptions).filter(name => pluginsOptions[name]).map(_async$1(function (name) {
        ...
        const fileName = fileNameTemplate.replace(/\[min\]/, minPlaceholder) // The `[ext]` placeholder no longer makes sense
          ...
          return _catch(function () {
            return _invokeIgnored(function () {
              if (options.concurrent) {
                ...
                return _awaitIgnored(pWaterfall_1(tasks.map(task => () => {
                  return _this2.build(task, context, options.write);
                }), context));
              }
            });
